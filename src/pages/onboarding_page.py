    ...
    return ft.Row(
        ...
        controls=[
            ft.Container(
                ...
                alignment=CENTER,
                content=ft.Icon(ft.Icons.NAVIGATION_ROUNDED, size=ts(24) if is_desktop else 22, color=ft.Colors.WHITE),
            ),
            ft.Text(
                "Белпомощник",
                size=ts(22) if is_desktop else 20,
...
def _search_preview(is_desktop: bool, width: int | None = None) -> ft.Container:
    return ft.Container(
        ...
        content=ft.Row(
            ...
            vertical_alignment=ft.CrossAxisAlignment.CENTER,
            controls=[
                ft.Icon(ft.Icons.SEARCH, size=ts(24), color=APP_COLORS["muted"]),
                ft.Text(
                    "Паспорт, ЖКХ, налоги, прописка...",
                    size=ts(15) if is_desktop else 14,
            ...
            controls=[
                ...
                ft.Column(
                    ...
                    controls=[
                        ft.Text(
                            item["title"],
                            size=ts(17) if is_desktop else 16,
                            weight=ft.FontWeight.W_900,
                            color=APP_COLORS["text"],
                        ),
                        ft.Text(
                            item["description"],
                            size=ts(13) if is_desktop else 12,
                ...
                ft.Text(item["badge"], size=ts(18), weight=ft.FontWeight.W_900, color=APP_COLORS["muted2"]),
            ],
        ),
        padding=16,
        border_radius=18,
...
def _desktop_hero(on_start) -> ft.Container:
    ...
    content = ft.Column(
        ...
        controls=[
            ...
            ft.Text(description, size=ts(17), color=APP_COLORS["muted"]),
...
def _mobile_hero(on_start) -> ft.Container:
    ...
    controls: list[ft.Control] = [
        ...
        badge("mobile-first · РБ", "blue"),
        ft.Text(title, size=ts(30), weight=ft.FontWeight.W_900, color=APP_COLORS["text"]),
