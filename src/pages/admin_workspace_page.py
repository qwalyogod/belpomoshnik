...
from theme.app_theme import APP_COLORS, padding_symmetric, padding_only
...
def _tree_row(
    *,
    ...
    leading_icon = ft.Icon(icon, size=16, color=icon_color)
...
def _sidebar(
    *,
    ...
    header = ft.Row(
        controls=[
            ft.Icon(ft.Icons.FOLDER_OPEN, size=18, color=APP_COLORS["primary"]),
            ft.Text(
                "EXPLORER",
                size=12,
...
def _editor_preview(state: dict, workspace_state: dict) -> ft.Control:
    ...
    notice = ft.Container(
        content=ft.Row(
            controls=[
                ft.Icon(ft.Icons.INFO_OUTLINE, size=18, color=APP_COLORS["primary"]),
                ft.Text(
